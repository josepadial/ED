...
class Diccionario{
	  ...
	  public:
		...
		*/
		...
		list<string> PalabrasPorLetra(char l){
		    typename list<data<T,U> >::iterator it = datos.begin();
		    list<string> aux;
		    string saux;
		    bool encontrado = false;
		    while(!encontrado && it != datos.end()){
		       saux = (*it).clave;
		       if(saux.front() == l){
		         	encontrado = true;
		         	aux.push_back(saux);
		...
		list<string> PalabrasPorSubcadena(string subcadena){
	    typename list<data<T,U> >::iterator it = datos.begin();
	    list<string> aux;
	    string saux;
	    while(it != datos.end()){
	       	saux = (*it).clave;
	       	char b = saux.find(subcadena);
	       	if(b != string::npos){
	        	aux.push_back(saux);
	       	}
	       	++it;
		...
		string PalabraPorDefinicion(string midefinicion){
	    	typename list<data<T,U> >::iterator it = datos.begin();
	    	typename list<string> ::iterator it_i;
	    	bool encontrado = false;
	    	string aux;
	    	while(it != datos.end() && !encontrado){
	       		for(it_i = (*it).info_asoci.begin(); it_i != (*it).info_asoci.end(); ++it_i){
	         		if((*it_i) == midefinicion){
	           			aux = (*it).clave;
	           			encontrado = true;
	         		}
	       		}
	  	...
	  	void borraElUltimoSignificado(const T& clave){
	        typename list<data<T,U> >::iterator it;
	        if(Esta_Clave(clave,it))
	         (*it).info_asoci.pop_back();
	        else{
	          cout<<"No existe la Clave dada"<<endl;
	        }

	    }

	    void borrarPrimerSignificado(const T& clave){
	       typename list<data<T,U> >::iterator it;
	       if(Esta_Clave(clave,it))
	        (*it).info_asoci.pop_front();
	        else{
	          cout<<"No existe la Clave dada"<<endl;
	        }
	    }

	    void borrarUltimaPalabra(){
        	datos.pop_back();
	    }

	     void borrarPrimeraPalabra(){
	        datos.pop_front();
	    }
