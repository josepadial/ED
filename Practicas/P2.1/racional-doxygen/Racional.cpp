...
//Impresion double en cout
void  Racional::printDouble(){
	double a;
	a = (double)this->num/this->den;
	cout << " El resultado en decimal es: " << a;
}
...
Racional Racional::operator+(const Racional &r){
  return Racional(num*r.den+den*r.num, den*r.den).simplifica();
}

//Operador *
Racional Racional::operator*(const Racional &r){
  return Racional(num*r.num, den*r.den).simplifica();
}

//Operador /
Racional Racional::operator/(const Racional &r){
  return Racional(num*r.den, den*r.num).simplifica();
}
